...
func TestGenericTypesInExpressions(t *testing.T) {
	tests := []struct {
		input    string
		expected string
	}{
		{"return Vec<int>", "Vec<int>"},
	...
	for _, tt := range tests {
		t.Run(tt.input, func(t *testing.T) {
			...
			assert.Equal(t, tt.expected, ret.Value.String())
		})
	}
}

func TestNoneConstructor(t *testing.T) {
	stmts := parseFunctionBody(t, "return None")
	ret := stmts[0].(*ast.ReturnStmt)
	call, ok := ret.Value.(*ast.CallExpr)
	require.True(t, ok)
	assert.Equal(t, "None", call.Function.String())
	assert.Empty(t, call.Arguments)
}

func TestIfExpressionEdgeCases(t *testing.T) {
	tests := []string{
		"if x > 5 { => true }", // no parens
		"if true { => 1 } else { => 0 }",
		"if (false) { => 42 }",
		// "if (x) { => y } else if (z) { => w }", // TODO: else-if not yet supported, but test current behavior
	}

	for _, input := range tests {
		t.Run(input, func(t *testing.T) {
			stmts := parseFunctionBody(t, "return "+input)
			_, ok := stmts[0].(*ast.ReturnStmt).Value.(*ast.IfExpr)
			assert.True(t, ok)
		})
	}
}
...
func TestStructLiteralErrors(t *testing.T) {
	inputs := []string{
		"5{x: 10}",         // non-identifier left side
		"Point{x 10}",      // missing colon
		"Point{x: 1, y: 2", // missing }
	}

	for _, in := range inputs {
		t.Run(in, func(t *testing.T) {
			l := lexer.New("fn test() { return " + in + " }")
			p := New(l)
			p.ParseProgram()
			assert.NotEmpty(t, p.Errors())
		})
...
func TestCallWithModifiers(t *testing.T) {
	input := `fn test() {
		foo(mut x, own y, z)
		return bar(mut a.b, own c[0])
	}`
	program := parseProgram(t, input)
	fn := program.Decls[0].(*ast.FnDecl)
	call1 := fn.Body.Statements[0].(*ast.ExprStmt).Value.(*ast.CallExpr)

	assert.True(t, call1.Arguments[0].Mut)
	assert.True(t, call1.Arguments[1].Own)
	assert.False(t, call1.Arguments[2].Mut)
	assert.False(t, call1.Arguments[2].Own)
}
...
func TestSliceExpressions(t *testing.T) {
	cases := []struct {
		...
		{"arr[:10]", false, true},
		{"arr[2:5]", true, true},
		{"arr[i+1 : j-1]", true, true},
	}

	for _, c := range cases {
		t.Run(c.input, func(t *testing.T) {
			stmts := parseFunctionBody(t, "return "+c.input)
			slice, ok := stmts[0].(*ast.ReturnStmt).Value.(*ast.SliceExpr)
			require.True(t, ok)
			if c.low {
...
func TestForLoopVariants(t *testing.T) {
	tests := []string{
		...
		"for { }",                             // infinite
		"for mut i := 0; i < 5; i = i + 1 {}", // mut init
	}
...
func TestErrorCapping(t *testing.T) {
	var b strings.Builder
	for i := 0; i < 40; i++ {
		b.WriteString("fn bad() { x := @ }\n")
	}
	l := lexer.New(b.String())
	p := NewWithMaxErrors(l, 10)
	p.ParseProgram()

	errs := p.Errors()
	assert.GreaterOrEqual(t, len(errs), 10)
	assert.Contains(t, errs[len(errs)-1], "too many errors")
}

func TestSumTypeDeclarations(t *testing.T) {
	tests := []struct {
		name             string
		input            string
		...
		{
			name:             "sum type with payload",
			input:            `type Result = | Ok { value int } | Err { msg string }`,
			expectedVariants: 2,
		},
		{
			name:             "empty variant + payload",
			input:            `type Option = | None | Some { val int }`,
			expectedVariants: 2,
		},
	}
...
func TestDeclParserErrorPaths(t *testing.T) {
	cases := []string{
		"type Point = { x int y int }", // missing comma
		"type Color = | Red {",         // unclosed payload
		"type Foo = | Bar { x int",     // missing }
		"fn bad( x int, ) {}",          // trailing comma in params
		"type Invalid = @",             // garbage after =
	}
...
func TestSynchronizeAndStatementEnd(t *testing.T) {
	input := `
fn test() int {
	x := 1
	y := @   // error
	z := 3   // should still parse after recovery
}
`
	program := parseProgramNoCheck(t, input) // helper that doesn't call checkParserErrors
	require.Len(t, program.Decls, 1)
	fn := program.Decls[0].(*ast.FnDecl)
	assert.Len(t, fn.Body.Statements, 3) // should recover and parse z
}

func parseProgramNoCheck(t *testing.T, input string) *ast.Program {
	t.Helper()
	l := lexer.New(input)
	p := New(l)
...
func TestIfExpressionNoParensAndElse(t *testing.T) {
	input := `if x > 0 { => 1 } else { => 0 }`
	stmts := parseFunctionBody(t, "return "+input)
	ifExpr := stmts[0].(*ast.ReturnStmt).Value.(*ast.IfExpr)
	assert.NotNil(t, ifExpr.Alternative)
}

func TestStructLiteralErrorPaths(t *testing.T) {
	input := `fn test() { return 5{x: 10} }` // non-identifier left side
	l := lexer.New(input)
	p := New(l)
	p.ParseProgram()
	assert.NotEmpty(t, p.Errors())
	// Should have hit the !ok branch in parseStructLiteral
}

func TestIndexExpressionEdgeCases(t *testing.T) {
	cases := []string{
		"arr[:]",   // empty low + high
		"arr[ : ]", // spaces around colon
		"arr[1:]",  // low only
		"arr[:10]", // high only
	}
	...
	for _, c := range cases {
		t.Run(c, func(t *testing.T) {
			stmts := parseFunctionBody(t, "return "+c)
			_, ok := stmts[0].(*ast.ReturnStmt).Value.(*ast.SliceExpr)
			assert.True(t, ok)
		})
	}
}
