...
std::string Type::mangledName(const SymbolTable& sym) const
{
    // Similar to toString, but replacing special characters with underscores
    // to satisfy the backend's ABI expectations.
    std::string base = toString(sym);
    for (char& c : base) {
        if (c == '<' || c == '>' || c == ',' || c == ' ' || c == '[' || c == ']' || c == '('
            || c == ')') {
            c = '_';
