...
void compileTerminator(CodegenContext &ctx, const mir::Terminator &term) {
  try {
    std::visit(
        [&](auto &&arg) {
          ...
          if constexpr (std::is_same_v<T, mir::ReturnTerminator>) {
            ...
            if (arg.resume_block.empty() || arg.cleanup_block.empty() || arg.suspend_block.empty())
              ctx.Error.fatal("CoroSuspendTerminator is missing a target block ID!");
    ...
    ctx.Error.fatal(std::string("C++ Exception in compileTerminator: ") + e.what());
  }
}
