...
void compileCoroutinePrologue(CodegenContext &ctx, llvm::Function *F) {
  ...
  llvm::Function *coroSizeFn =
      llvm::Intrinsic::getDeclaration(Module.get(), llvm::Intrinsic::coro_size,
...
void compileFunction(CodegenContext &ctx, const nlohmann::json &fn) {
  ...
  llvm::Function *F = llvm::Function::Create(
      FT, llvm::Function::ExternalLinkage, llName, *Module);
  ...
  for (auto &Arg : F->args()) {
    ...
    llvm::AllocaInst *alloca =
        Builder->CreateAlloca(pType, nullptr, std::string(pName));
...
void compileProgram(CodegenContext &ctx, const nlohmann::json &ast) {
  ...
  if (ast.contains("node_type"))
    nodeType = ast["node_type"].get<std::string_view>();
  ...
  for (const auto &decl : ast["decls"]) {
    ...
    if (decl.contains("node_type"))
      declNodeType = decl["node_type"].get<std::string_view>();
